...
public class Simulator {
	...
	public double runSimulation() {
		...
		Integer next_job_ind = 1; // Then next job to add to queue/to server if empty
		...
		for (int i = 990000; i<num_jobs; i++) {
			Double Tarrival = jobs.get(i).get(1);
			Double Tdeparture = jobs.get(i).get(2);
			total_rp += Tdeparture-Tarrival;
		}
		
		return total_rp/num_jobs;
	}
}
